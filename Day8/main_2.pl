            ...
            for(my $row_continue = $row; $row_continue < scalar @records; $row_continue++){
                for(my $column_continue = 0; $column_continue < scalar @record; $column_continue++){
                    ...
                    if($is_first_cycle){
                        ...
                        if($column_continue > scalar @record){
                            $column_continue = 0;
                            $row_continue = $row + 1;
                            if($row_continue > scalar @records){
                                last;
                            }
                        }
                    }
                    ...
                    if(${$records[$row_continue]}[$column_continue] eq $cur_antenna){
                        ...
                        if($column >= $column_continue){
                            while($sub_of_vector_right_y >= 0 && $sub_of_vector_right_x >= 0 && $sub_of_vector_right_y < scalar @records && $sub_of_vector_right_x < scalar @record){
                                $result{"$sub_of_vector_right_y,$sub_of_vector_right_x"} = 1;
                                $sub_of_vector_right_y = $sub_of_vector_right_y - $new_y;
                                $sub_of_vector_right_x = $sub_of_vector_right_x + $new_x;
                            }
                            while($add_to_vector_right_y < scalar @records && $add_to_vector_right_x < scalar @record && $add_to_vector_right_y >= 0 && $add_to_vector_right_x >= 0){
                                $result{"$add_to_vector_right_y,$add_to_vector_right_x"} = 1;
                                $add_to_vector_right_y = $add_to_vector_right_y + $new_y;
                                $add_to_vector_right_x = $add_to_vector_right_x - $new_x;
                            }
                        }else{
                            while($sub_of_vector_left_y >= 0 && $sub_of_vector_left_x >= 0 && $sub_of_vector_left_y < scalar @records && $sub_of_vector_left_x < scalar @record){
                                $result{"$sub_of_vector_left_y,$sub_of_vector_left_x"} = 1;
                                $sub_of_vector_left_y = $sub_of_vector_left_y - $new_y;
                                $sub_of_vector_left_x = $sub_of_vector_left_x - $new_x;
                            }
                            while($add_to_vector_left_y < scalar @records && $add_to_vector_left_x < scalar @record && $add_to_vector_left_y >= 0 && $add_to_vector_left_x >= 0){
                                $result{"$add_to_vector_left_y,$add_to_vector_left_x"} = 1;
                                $add_to_vector_left_y = $add_to_vector_left_y + $new_y;
                                $add_to_vector_left_x = $add_to_vector_left_x + $new_x;
                            }
                        }
                    }
                }
